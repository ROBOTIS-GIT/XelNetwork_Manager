...
bool cmdBegin(cmd_t *p_cmd, uint8_t ch,char *port_name, uint32_t baud)
{
  p_cmd->ch   = ch;
  p_cmd->baud = baud;
  p_cmd->init = true;
  ...
  if (uartOpen(ch, port_name, baud) == 0)
    return true;
  else
    return false;
}
...
err_code_t cmdSendCmdRxResp(cmd_t *p_cmd, uint8_t cmd, uint8_t *p_data, uint32_t length, uint32_t timeout)
{
